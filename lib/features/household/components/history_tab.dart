...
class _ImpactCard extends StatelessWidget {
  const _ImpactCard({required this.bookings});
  final List<Map<String, dynamic>> bookings;

  @override
  Widget build(BuildContext context) {
    final totalSpent = bookings.fold<double>(0, (sum, b) => sum + Fmt.toDouble(b['totalAmount']));
    final kgRecycled = bookings.fold<double>(0, (sum, b) {
      final cat = (b['wasteCategory'] as String? ?? '').toUpperCase();
      final recyclable = ['PLASTIC', 'GLASS', 'METAL', 'ORGANIC'].contains(cat);
      return sum + (recyclable ? Fmt.toDouble(b['actualWeightKg'] ?? b['estimatedWeightKg']) : 0);
    });
    ...
    return HCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          SvgPicture.asset('assets/household_assets/history/carbon_saved.svg', height: 56),
          const SizedBox(width: 14),
          Expanded(child: Text('Your impact record', style: HouseholdType.title.copyWith(color: Colors.white, fontSize: 18))),
        ]),
        const SizedBox(height: 16),
        Row(children: [
          _StatChip(label: 'Pickups', value: '${bookings.length}', icon: 'pickup'),
          const SizedBox(width: 10),
          _StatChip(label: 'Spent', value: 'GHS ${totalSpent.toStringAsFixed(0)}', icon: 'wallet'),
          const SizedBox(width: 10),
          _StatChip(label: 'Recycled', value: '${kgRecycled.toStringAsFixed(1)} kg', icon: 'carbon'),
        ]),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          HIcon(icon, size: 18, color: Colors.white70),
          const SizedBox(height: 6),
          Text(value, style: HouseholdType.number.copyWith(color: Colors.white, fontSize: 14)),
          const SizedBox(height: 2),
          Text(label, style: HouseholdType.caption.copyWith(color: Colors.white60, fontSize: 11)),
        ]),
      ),
    );
  }
