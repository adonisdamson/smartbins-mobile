...
class _BookFooter extends StatelessWidget {
  const _BookFooter({required this.step, required this.loading, required this.canNext, required this.total, required this.onBack, required this.onNext});
  final int step;
  final bool loading;
  final bool canNext;
  final double total;
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(children: [
        ...
        Expanded(
          child: HButton(
            label: step == 4 ? 'Confirm & Pay GHS ${total.toStringAsFixed(2)}' : 'Continue',
